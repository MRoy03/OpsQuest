...
const QUICK_COMMANDS: Record<string, { label: string; type: 'ps1' | 'bat'; desc: string; cmd: string }[]> = {
  ...
  'Cmd Prompt': [
    { label: 'System Info',         type: 'bat', desc: 'Full system info (OS, CPU, RAM, NICs)',  cmd: 'systeminfo' },
    { label: 'Task List (verbose)', type: 'bat', desc: 'All running tasks with memory usage',    cmd: 'tasklist /v' },
    { label: 'Services List',       type: 'bat', desc: 'All services and their status',          cmd: 'sc query type= all state= all' },
    { label: 'IP Config',           type: 'bat', desc: 'All adapters IPs, MACs, DNS',            cmd: 'ipconfig /all' },
    { label: 'Netstat (All)',        type: 'bat', desc: 'All TCP/UDP connections with PID',       cmd: 'netstat -ano' },
    { label: 'Netstat Listening',   type: 'bat', desc: 'Listening ports with PID',               cmd: 'netstat -ano | findstr LISTENING' },
    { label: 'ARP Table',           type: 'bat', desc: 'IP-to-MAC mapping',                      cmd: 'arp -a' },
    { label: 'Route Print',         type: 'bat', desc: 'Routing table',                          cmd: 'route print' },
    { label: 'Dir C:\\',            type: 'bat', desc: 'Root drive listing with sizes',          cmd: 'dir C:\\ /a' },
    { label: 'Environment Vars',    type: 'bat', desc: 'All system environment variables',       cmd: 'set' },
    { label: 'User Info',           type: 'bat', desc: 'Current user account details',           cmd: 'whoami /all' },
    { label: 'Logged On Users',     type: 'bat', desc: 'Users currently logged on',              cmd: 'query user' },
    { label: 'Group Policy',        type: 'bat', desc: 'Applied group policies (verbose)',       cmd: 'gpresult /v' },
    { label: 'Disk Usage',          type: 'bat', desc: 'Drive letters and free space',           cmd: 'wmic logicaldisk get caption,freespace,size,volumename' },
    { label: 'Installed Hotfixes',  type: 'bat', desc: 'Windows update hotfix list',             cmd: 'wmic qfe list brief' },
    { label: 'Startup Items',       type: 'bat', desc: 'Programs in startup (WMIC)',             cmd: 'wmic startup list full' },
    { label: 'Shared Folders',      type: 'bat', desc: 'SMB shares on this machine',             cmd: 'net share' },
    { label: 'Open Sessions',       type: 'bat', desc: 'Remote sessions to this machine',        cmd: 'net session' },
    { label: 'Hosts File',          type: 'bat', desc: 'Contents of C:\\Windows\\System32\\drivers\\etc\\hosts', cmd: 'type C:\\Windows\\System32\\drivers\\etc\\hosts' },
    { label: 'DNS Flush',           type: 'bat', desc: 'Flush the DNS resolver cache',           cmd: 'ipconfig /flushdns' },
  ],
  BIOS: [
    { label: 'BIOS Details',        type: 'ps1', desc: 'Manufacturer, version, release date, serial', cmd: "Get-CimInstance Win32_BIOS | Select-Object Manufacturer,Name,Version,SMBIOSBIOSVersion,ReleaseDate,SerialNumber,BIOSVersion | Format-List" },
    { label: 'UEFI / BIOS Mode',    type: 'ps1', desc: 'Whether booted in UEFI or legacy BIOS', cmd: "$fw = (Get-ItemProperty 'HKLM:\\SYSTEM\\CurrentControlSet\\Control' -Name 'PEFirmwareType' -EA SilentlyContinue).PEFirmwareType; if ($fw -eq 2) { 'Boot mode: UEFI' } elseif ($fw -eq 1) { 'Boot mode: Legacy BIOS' } else { 'Boot mode: Unknown' }" },
    { label: 'Secure Boot Status',  type: 'ps1', desc: 'Whether Secure Boot is enabled',        cmd: "try { $sb = Confirm-SecureBootUEFI; \"Secure Boot: $(if ($sb) {'Enabled'} else {'Disabled'})\" } catch { 'Secure Boot: Not supported or legacy BIOS' }" },
    { label: 'TPM Status',          type: 'ps1', desc: 'TPM chip presence, version, readiness',  cmd: "Get-Tpm | Select-Object TpmPresent,TpmReady,TpmEnabled,TpmActivated,TpmOwned,ManufacturerId,ManagedAuthLevel | Format-List" },
    { label: 'TPM Version',         type: 'ps1', desc: 'TPM spec version (1.2 or 2.0)',          cmd: "Get-CimInstance -Namespace 'root/cimv2/Security/MicrosoftTpm' Win32_Tpm | Select-Object SpecVersion,PhysicalPresenceVersionInfo,IsEnabled_InitialValue,IsActivated_InitialValue | Format-List" },
    { label: 'BitLocker Status',    type: 'ps1', desc: 'Drive encryption status per volume',     cmd: "Get-BitLockerVolume | Select-Object MountPoint,EncryptionMethod,VolumeStatus,ProtectionStatus,LockStatus,EncryptionPercentage | Format-Table -AutoSize" },
    { label: 'System Board Info',   type: 'ps1', desc: 'Motherboard make, model, serial',        cmd: "Get-CimInstance Win32_BaseBoard | Select-Object Manufacturer,Product,Version,SerialNumber | Format-List" },
    { label: 'System Model',        type: 'ps1', desc: 'OEM system model and serial number',     cmd: "Get-CimInstance Win32_ComputerSystem | Select-Object Manufacturer,Model,SystemSKUNumber; Get-CimInstance Win32_SystemEnclosure | Select-Object SerialNumber,SMBIOSAssetTag | Format-List" },
    { label: 'Virtualization',      type: 'ps1', desc: 'CPU virt support and Hyper-V status',   cmd: "Get-CimInstance Win32_Processor | Select-Object Name,VirtualizationFirmwareEnabled; (Get-WindowsOptionalFeature -FeatureName Microsoft-Hyper-V-All -Online -EA SilentlyContinue) | Select-Object FeatureName,State | Format-List" },
    { label: 'BIOS Boot Order',     type: 'bat', desc: 'Boot configuration (bcdedit)',           cmd: 'bcdedit /enum all' },
    { label: 'BIOS Update Check',   type: 'ps1', desc: 'Current vs available BIOS via WU',      cmd: "Get-WmiObject -Namespace 'root\\cimv2\\mdm\\dmmap' -Class MDM_BIOS -EA SilentlyContinue | Format-List; Get-CimInstance Win32_BIOS | Select-Object SMBIOSBIOSVersion,ReleaseDate | Format-List" },
    { label: 'Power Plan',          type: 'ps1', desc: 'Active power plan (performance/balanced)', cmd: "Get-CimInstance Win32_PowerPlan -Namespace root\\cimv2\\power | Where-Object {$_.IsActive} | Select-Object ElementName,Description | Format-List" },
    { label: 'Wake-on-LAN',         type: 'bat', desc: 'Check if Wake-on-LAN is configured',    cmd: 'powercfg /devicequery wake_armed' },
    { label: 'Hyper-V Features',    type: 'ps1', desc: 'Hyper-V role and feature state',        cmd: "Get-WindowsOptionalFeature -Online | Where-Object {$_.FeatureName -like '*Hyper-V*'} | Select-Object FeatureName,State | Format-Table -AutoSize" },
  ],
}
